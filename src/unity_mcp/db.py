...
Two logical indexes:
- api_records   + api_fts    -> precise Unity API/symbol lookup
- guide_records + guide_fts  -> conceptual / how-to / tutorial lookup
"""
...
from .config import DB_PATH, MAX_CONTENT_LENGTH
from .models import ApiRecord, GuideRecord


_SCHEMA_SQL = """
-- ===========================================================================
-- API records: ScriptReference symbols (classes, structs, enums, methods, ...)
-- ===========================================================================
...
-- FTS5 index tuned for symbol/title precision: short identifier-y fields are
-- weighted higher than free-form prose at query time via bm25(weights).
...
CREATE TRIGGER IF NOT EXISTS api_records_ai AFTER INSERT ON api_records BEGIN
    INSERT INTO api_fts(rowid, symbol_name, title, class_name, namespace, signature, summary, remarks, content_text)
    VALUES (new.id, new.symbol_name, new.title, new.class_name, new.namespace, new.signature, new.summary, new.remarks, new.content_text);
END;
CREATE TRIGGER IF NOT EXISTS api_records_ad AFTER DELETE ON api_records BEGIN
    INSERT INTO api_fts(api_fts, rowid, symbol_name, title, class_name, namespace, signature, summary, remarks, content_text)
    VALUES ('delete', old.id, old.symbol_name, old.title, old.class_name, old.namespace, old.signature, old.summary, old.remarks, old.content_text);
END;
...
-- ===========================================================================
-- Guide records: Manual / conceptual / tutorial / overview pages
-- ===========================================================================
...
-- FTS5 index tuned for natural-language conceptual matching: title and
-- key_topics carry the strongest signal, then summary, then prose.
...
CREATE TRIGGER IF NOT EXISTS guide_records_ai AFTER INSERT ON guide_records BEGIN
    INSERT INTO guide_fts(rowid, title, key_topics, summary, content_text)
    VALUES (new.id, new.title, new.key_topics_json, new.summary, new.content_text);
END;
CREATE TRIGGER IF NOT EXISTS guide_records_ad AFTER DELETE ON guide_records BEGIN
    INSERT INTO guide_fts(guide_fts, rowid, title, key_topics, summary, content_text)
    VALUES ('delete', old.id, old.title, old.key_topics_json, old.summary, old.content_text);
END;
...
def upsert_api_record(conn: sqlite3.Connection, rec: ApiRecord) -> int:
    ...
    if row:
        conn.execute(
            """UPDATE api_records SET
                title=?, symbol_name=?, class_name=?, namespace=?, member_type=?,
                signature=?, parameters_json=?, returns_text=?, summary=?, remarks=?,
                content_text=?, source_html_path=?,
                last_indexed_at=CURRENT_TIMESTAMP
            WHERE id=?""",
            (
                rec.title, rec.symbol_name, rec.class_name, rec.namespace, rec.member_type,
                rec.signature, rec.parameters_json, rec.returns_text, rec.summary, rec.remarks,
                content_text, rec.source_html_path,
                row["id"],
            ),
        )
        return row["id"]
    ...
    cursor = conn.execute(
        """INSERT INTO api_records (
            title, relative_path, symbol_name, class_name, namespace, member_type,
            signature, parameters_json, returns_text, summary, remarks,
            content_text, source_html_path
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            rec.title, rec.relative_path, rec.symbol_name, rec.class_name, rec.namespace,
            rec.member_type, rec.signature, rec.parameters_json, rec.returns_text,
            rec.summary, rec.remarks, content_text, rec.source_html_path,
        ),
    )
...
def upsert_guide_record(conn: sqlite3.Connection, rec: GuideRecord) -> int:
    ...
    if row:
        conn.execute(
            """UPDATE guide_records SET
                title=?, guide_type=?, summary=?, content_text=?, key_topics_json=?,
                source_html_path=?, last_indexed_at=CURRENT_TIMESTAMP
            WHERE id=?""",
            (
                rec.title, rec.guide_type, rec.summary, content_text, rec.key_topics_json,
                rec.source_html_path, row["id"],
            ),
        )
        return row["id"]
    ...
    cursor = conn.execute(
        """INSERT INTO guide_records (
            title, relative_path, guide_type, summary, content_text, key_topics_json,
            source_html_path
        ) VALUES (?, ?, ?, ?, ?, ?, ?)""",
        (
            rec.title, rec.relative_path, rec.guide_type, rec.summary, content_text,
            rec.key_topics_json, rec.source_html_path,
        ),
    )
    return cursor.lastrowid
